...
const stores = computed(() => (seed?.stores || []).filter(Boolean))
...
const storeId   = ref(stores.value[0]?.id || '')
const dateFrom  = ref('')
const dateTo    = ref('')
...
/* 取得 seed 可用的日期界線（避免初次載入沒資料） */
function getSeedDateBounds(){
  const all = []
  for (const bucket of (storeWasteUsage.value || [])) {
    for (const rec of (bucket.records || [])) {
      if (rec?.date) all.push(rec.date)
    }
  }
...
onMounted(()=>{
  const bounds = getSeedDateBounds()
  if (bounds){
    dateFrom.value = bounds.min
...
const dailyRecords = computed(()=>{
  ...
  const ids = scope.value==='all' ? stores.value.map(s=>s.id) : [storeId.value]

  const all = []
  for (const sid of ids) {
    const bucket = storeWasteUsage.value.find(x => x.storeId === sid)
    if (!bucket) continue
    for (const rec of bucket.records || []) {
      const d = rec?.date || ''
      if (d >= from && d <= to) {
        all.push({ storeId: sid, ...rec })
      }
    }
  }
  return all
...
async function exportPNG(){ await pieRef.value?.downloadPNG?.(`boss-pie-${metric.value}-${groupBy.value}.png`) }

function escapeCSV(v){ const s=String(v??''); return /[,"\n]/.test(s)?`"${s.replace(/"/g,'""')}"`:s }
function exportCSV(){
  const storeLabel = scope.value==='all' ? '全部店面' : (stores.value.find(s=>s.id===storeId.value)?.name || '—')
  const rows = [
    ['店面範圍', storeLabel],
  ...
  const url = URL.createObjectURL(blob)
  const a=document.createElement('a'); a.href=url; a.download='boss-report.csv'; a.click()
