  ...
  <section class="inv-page">
    ...
    <div class="inv-grid">
      ...
      <aside class="inv-side">
        <div class="side-tools">
          ...
          <div class="row gap">
            <button class="btn primary w-full" @click="openQuick">
              ＋ 產生邀請碼
            </button>
          </div>
          ...
          <div class="chips">
            <button
              class="chip"
              :class="{ on: statusFilter.has('active') }"
              @click="toggle(statusFilter, 'active')"
            >
              #啟用中
            </button>
            <button
              class="chip"
              :class="{ on: statusFilter.has('revoked') }"
              @click="toggle(statusFilter, 'revoked')"
            >
              #已撤銷
            </button>
            <button
              class="chip"
              :class="{ on: statusFilter.has('expired') }"
              @click="toggle(statusFilter, 'expired')"
            >
              #已到期
            </button>
            <button
              class="chip"
              :class="{ on: statusFilter.has('used') }"
              @click="toggle(statusFilter, 'used')"
            >
              #已用罄
            </button>
          </div>
        ...
        <div class="side-list">
          ...
          >
            <div class="badge" :class="resolveStatus(c)">
              {{ statusText(resolveStatus(c)) }}
            </div>
            <div class="grow">
              <div class="name ell">
              ...
              <div class="muted small ell">
                {{ roleName(c.role) }}｜可用 {{ c.usesLeft }}/{{ c.usesAllowed }}｜{{
                  c.expiresAt ? '到期 ' + c.expiresAt : '永不過期'
                }}
              </div>
              <div class="muted tiny ell" v-if="c.note">
                {{ c.note }}
              </div>
              <div class="muted tiny ell" v-if="c.displayName">
                預設姓名：{{ c.displayName }}
              </div>
            </div>
          </div>
          <p v-if="!filteredCodes.length" class="muted center">
            沒有符合的邀請碼
          </p>
        </div>
      </aside>
        ...
        <div class="form-cols">
          ...
          <section class="card-lite">
            ...
            <div class="row gap mt-8">
              ...
              <input
                ...
                min="1"
                class="input w120"
                v-model.number="editing.usesAllowed"
              >
            ...
            <div class="row gap mt-8">
              ...
              <input
                class="input grow"
                v-model.trim="editing.note"
                placeholder="給管理者看的描述…"
              >
            </div>

            <!-- 🔹 新增：預設姓名（登入時顯示給員工） -->
            <div class="row gap mt-8">
              <label class="label">預設姓名</label>
              <select
                class="input w220"
                v-model="displayNamePreset"
                @change="applyDisplayNamePreset"
              >
                <option
                  v-for="p in NAME_PRESETS"
                  :key="p.id"
                  :value="p.id"
                >
                  {{ p.label }}
              ...
              <input
                class="input grow"
                v-model.trim="editing.displayName"
                placeholder="例如：王小明、門市店長、中央廚房人員…"
              >
            </div>
            <p class="muted tiny mt-8">
              員工透過邀請碼註冊時會看到這個
              <strong>預設姓名</strong>，可協助確認身份，也可在註冊表單中自行修改。
            </p>
            ...
            <div class="row gap mt-8">
              ...
              <input
                class="input w220"
                v-model.trim="editing.emailDomain"
                placeholder="如 example.com（可留空）"
              >
              <label class="chk ml12">
                <input
                  type="checkbox"
                  v-model="editing.oneTimeUse"
                >
                <span></span>
              </label>
              <span class="muted">
          ...
          <section class="card-lite">
            ...
            <div class="perm-grid mt-8">
              ...
              >
                <input
                  type="checkbox"
                  v-model="editing.permissions[p.key]"
                >
                <span>{{ p.name }}</span>
              </label>
            </div>
          ...
          <section class="card-lite">
            ...
            <div class="row gap">
              ...
              >
              <button class="btn" @click="batchGenerate">
                ＋ 批次生成
              </button>
              <div class="muted small">
                批次會使用目前設定（權限 / 店面 / 限制 / 預設姓名 等）
              </div>
            </div>
            <div class="row gap mt-8">
              <button
    ...
    <transition name="fade">
      <div
        v-if="quickOpenFlag"
        class="modal"
        @click.self="quickOpenFlag = false"
      >
        <div class="sheet">
          ...
          <div class="sheet-body">
            <div class="row gap">
              ...
              <div class="seg">
                <button
                  :class="['segbtn', quick.scope === 'all' && 'active']"
                  @click="quick.scope = 'all'"
                >
                  全部店面
                </button>
                <button
                  :class="['segbtn', quick.scope === 'custom' && 'active']"
                  @click="quick.scope = 'custom'"
                >
                  指定店面
                </button>
              </div>
            ...
            <div class="row gap mt-8">
              ...
              <input
                ...
                min="1"
                class="input w120"
                v-model.number="quick.usesAllowed"
              >
            </div>
            ...
            <div class="row gap mt-8">
              ...
              <select
                class="input w220"
                v-model="quickDisplayNamePreset"
                @change="applyQuickDisplayNamePreset"
              >
                <option
                  v-for="p in NAME_PRESETS"
                  :key="'q-' + p.id"
              ...
              <input
                class="input grow"
                v-model.trim="quick.displayName"
                placeholder="例如：新進店員、晚班工讀…（可留空）"
              >
            </div>
          </div>
...
const NAME_PRESETS = [
  { id: '', label: '（不指定）', value: '' },
  { id: 'store-manager', label: '門市店長', value: '門市店長' },
  { id: 'store-staff', label: '門市員工', value: '門市員工' },
  { id: 'ck-staff', label: '中央廚房人員', value: '中央廚房人員' },
]
...
/* 預設姓名 preset 狀態 */
const displayNamePreset = ref('')
const quickDisplayNamePreset = ref('')
...
const inviteLink = computed(() => {
  const origin =
    typeof location !== 'undefined' && location.origin
      ? location.origin
      : ''
  return `${origin}/signup?code=${encodeURIComponent(
    editing.code || '',
  )}`
})
...
const filteredCodes = computed(() => {
  ...
  return codes.value
    .filter(
      (c) => !statusFilter.size || statusFilter.has(resolveStatus(c)),
    )
...
function selectCode(id) {
  ...
  displayNamePreset.value =
    NAME_PRESETS.find((p) => p.value === editing.displayName)?.id ||
    ''
  snap.value = JSON.parse(JSON.stringify(editing))
}
function revert() {
  if (snap.value)
    Object.assign(editing, JSON.parse(JSON.stringify(snap.value)))
}
...
/* 預設姓名 preset 套用 */
function applyDisplayNamePreset() {
  const p = NAME_PRESETS.find((x) => x.id === displayNamePreset.value)
  if (!p) return
  editing.displayName = p.value
}
function applyQuickDisplayNamePreset() {
  const p = NAME_PRESETS.find((x) => x.id === quickDisplayNamePreset.value)
  if (!p) return
  quick.displayName = p.value
}

/* 店面 / 模板 */
function toggleStore(id) {
  const i = editing.storeIds.indexOf(id)
  if (i >= 0) editing.storeIds.splice(i, 1)
  else editing.storeIds.push(id)
}
...
async function toggleRevoke() {
  editing.revoked = !editing.revoked
  await save()
}
async function usesMinus() {
  if (editing.usesLeft > 0) editing.usesLeft--
  await save()
}
...
function openQuick() {
  ...
  quick.note = ''
  quick.displayName = ''
  quickDisplayNamePreset.value = ''
}
function toggleQuickStore(id) {
  const i = quick.storeIds.indexOf(id)
  if (i >= 0) quick.storeIds.splice(i, 1)
  else quick.storeIds.push(id)
}
async function confirmQuick() {
  const storeIds =
    quick.scope === 'all'
      ? stores.value.map((s) => s.id)
      : [...quick.storeIds]
