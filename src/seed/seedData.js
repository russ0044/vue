...
//
// ★ 新增（預覽/功能欄強化）
// - ui.menus：各角色功能選單（title/icon/route/perm）
// - ui.panels：首頁面板卡資訊（數字卡、快捷卡）
// - demo.empRequests / demo.empOrders：示例請貨與訂單，便於員工端預覽
// - dashboards.stats：全域統計數字（可供首頁或報表快速顯示）
// - orderSettings：訂單設定示例（老闆端 BossOrderSettings 預覽）
// - delivery.schedule：配送排程示例（門市/廚房端預覽）
// - reports.samples：報表示例摘要（Auditor/Boss/Employee 可讀）
...
export const seedData = {
  ...
  roleGroups: [
    ...
    },
    {
      ...
      permissions: [
        'inventory.view',
        'inventory.edit',   // ★ 店長可編輯庫存
        'ingredients.view',
        'orders.view',
        'orders.create',    // ★ 店長可送單/建立
        'delivery.view',
        'reports.view',
      ],
    },
   ...
   * ✅ 所有原本 2025-11-xx 的日期已調整為 2025-12-xx
   * ========================= */
  inventory: [
    // 台北店（示範店長開放權限的那一間）
    ...
    { storeId: 'hn-taipei', sku: 'SC-090', name: '蔥',                 qty:  8,  unit: '把',   exp: '2025-12-03' },
    { storeId: 'hn-taipei', sku: 'CI-091', name: '香菜',               qty:  5,  unit: '把',   exp: '2025-12-02' },
    { storeId: 'hn-taipei', sku: 'CU-092', name: '小黃瓜',             qty: 18,  unit: '條',   exp: '2025-12-03' },
    ...
    // 台中店
    { storeId: 'hn-taichung', sku: 'CK-001', name: '去骨雞腿（熟）',   qty: 14,  unit: '份',   exp: '2025-12-05' },
    { storeId: 'hn-taichung', sku: 'CK-002', name: '去骨雞胸（熟）',   qty:  8,  unit: '份',   exp: '2025-12-05' },
    ...
    { storeId: 'hn-taichung', sku: 'SC-090', name: '蔥',               qty:  5,  unit: '把',   exp: '2025-12-03' },
    { storeId: 'hn-taichung', sku: 'CI-091', name: '香菜',             qty:  3,  unit: '把',   exp: '2025-12-02' },
    { storeId: 'hn-taichung', sku: 'CU-092', name: '小黃瓜',           qty: 12,  unit: '條',   exp: '2025-12-03' },
    ...
    { storeId: 'hn-kaohsiung', sku: 'SC-090', name: '蔥',              qty:  6,  unit: '把',   exp: '2025-12-03' },
    { storeId: 'hn-kaohsiung', sku: 'CI-091', name: '香菜',            qty:  4,  unit: '把',   exp: '2025-12-02' },
    ...
    // 中央廚房：半成品/原物料（大宗）
    { storeId: 'central-kitchen', sku: 'PF-110', name: '去骨雞腿（真空包，生）', qty: 300, unit: '包',   exp: '2025-12-20' },
    { storeId: 'central-kitchen', sku: 'PF-111', name: '去骨雞胸（真空包，生）', qty: 200, unit: '包',   exp: '2025-12-20' },
    ...
    { storeId: 'central-kitchen', sku: 'GI-061', name: '老薑',                   qty:  60, unit: '公斤', exp: '2025-12-20' },
    { storeId: 'central-kitchen', sku: 'GA-060', name: '蒜頭',                   qty:  60, unit: '公斤', exp: '2025-12-20' },
  ...
  /* ==========================================================
   * ★ 新增：UI 功能欄與面板，用於各角色首頁/側欄預覽
   * - 僅為靜態配置，不影響既有權限判斷（可搭配 permissions 顯示/隱藏）
   * ========================================================== */
  ui: {
    menus: {
      boss: [
  ...
  /* ==========================================================
   * ★ 新增：Demo/預覽用的請貨與訂單（門市端）
   * EmpOrders.vue 使用 localStorage；此處為其他模組或匯出時可讀
   * ========================================================== */
  demo: {
    empRequests: [
      {
  ...
  dashboards: {
    stats: {
      ...
      safetyAlerts: 6,
      pendingRequests: 3,
      monthRevenueNTD: 865000, // 假資料：本月營收
      avgLeadTimeDays: 2.1,
    },
  },

  /* ==========================================================
   * ★ 新增：訂單設定與配送排程（老闆/門市/廚房預覽）
   * ========================================================== */
  orderSettings: {
    cutOffTime: '15:00',             // 每日截單時間
    defaultVendorId: 'vendor-central',
    allowCrossVendor: true,
    requireNoteWhenOverSafe: false,  // 數量高於安全存量是否強制填寫原因
    aiSuggestion: {
      enabled: true,
      horizonDays: 3,
      baseTrend: 1.06,
  ...
  delivery: {
    schedule: [
      { id: 'DL-001', date: '2025-11-10', routeName: '北區一線', vehicle: 'TRUCK-12', stops: [
        { idx: 1, storeId: 'hn-taipei',       eta: '10:00' },
        { idx: 2, storeId: 'hn-taichung',     eta: '13:30' },
      ]},
      { id: 'DL-002', date: '2025-11-11', routeName: '南區一線', vehicle: 'TRUCK-08', stops: [
        { idx: 1, storeId: 'hn-kaohsiung',    eta: '10:30' },
      ]},
    ],
  },
  ...
  reports: {
    samples: [
      { id: 'RPT-INV-01', title: '庫存週報',     period: '2025-11-03 ~ 2025-11-09', highlights: ['低於門檻 6 項', '周轉天數中位 11 天', '缺料主要集中於蔬菜類'] },
      { id: 'RPT-ORD-02', title: '訂單分析月報', period: '2025-11',                highlights: ['中央廚房供應占比 62%', '平均交期 2.1 天', '門市補貨高峰：週五'] },
    ],
  },
};
